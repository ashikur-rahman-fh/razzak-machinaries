...
import { TransactionCorrectForm } from '@/transactions/components/TransactionCorrectForm';
import { TransactionDetailSkeleton } from '@/transactions/components/TransactionDetailSkeleton';

export function TransactionCorrectPage() {
  ...
  return (
    <RequireAdminAuth>
      <AdminAppShell
        data-testid="transaction-correct-page"
      ...
      >
        ...
        {transaction && transaction.isCurrent && transaction.status === 'ACTIVE' ? (
          <TransactionCorrectForm transaction={transaction} />
        ) : null}
        ...
        {transaction && (!transaction.isCurrent || transaction.status !== 'ACTIVE') ? (
          <EmptyState
            title={
              <TranslatedText
                translationKey="transaction.correct.notAllowed"
                as="span"
                layout="inline"
              />
            }
